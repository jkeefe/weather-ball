...
const parseData = (xml) => {
  ...
  for (var layout of time_layouts) {
    ...
    const layout_name = layout.elements.filter((d) => d.name == "layout-key")[0]
      .elements[0].text;

    // get all the valid times in the layout
    const start_valid_times = layout.elements.filter(
      (d) => d.name == "start-valid-time",
    );
    const end_valid_times = layout.elements.filter(
      (d) => d.name == "end-valid-time",
    );

    // go get any weather data that uses this time layout
    const weather_sets = weather.elements.filter(
      (d) => d.attributes["time-layout"] == layout_name,
    );
    ...
    for (let index in start_valid_times) {
      ...
      for (let weather_set of weather_sets) {
        var key_name = weather_set.attributes.type
          ? weather_set.name + "_" + weather_set.attributes.type
          : weather_set.name;
        ...
        if (weather_set.name == "conditions-icon") {
          ...
          weather_values = weather_set.elements.filter(
            (d) => d.name == "icon-link",
          );
        } else {
          // just use the value
...
const setWeatherBall = async (preset) => {
  ...
  port.write(`{"ps": ${preset}}`, function (err) {
    ...
    console.log(
      `Serial message with preset ${preset} written to weather ball.`,
    );
  });
